...
不过最需要强调的便是它的可读性。一份使用 Markdown 格式撰写的文件应该可以直接以纯文字发佈，并且看起来不会像是由许多标签或是格式指令所构成。Markdown 语法受到一些既有 text-to-HTML 格式的影响，包括 [Setextopen ](http://docutils.sourceforge.net/mirror/setext.html)、[atxopen ](http://www.aaronsw.com/2002/atx/)、[Textileopen ](http://textism.com/tools/textile/)、[reStructuredTextopen ](http://docutils.sourceforge.net/rst.html)、[Grutatextopen ](http://www.triptico.com/software/grutatxt.html) 和 [EtTextopen ](http://ettext.taint.org/doc/)，然而最大灵感来源其实是纯文字的电子邮件格式。
...
类似的状况也会发生在 `<` 符号上，因为 Markdown 支持 [行内 HTML](https://theme-hope.vuejs.press/zh/cookbook/markdown/#行内-html) ，如果你是使用 `<` 符号作为 HTML 标签使用，那 Markdown 也不会对它做任何转换，但是如果你是写:
...
是的，这确实需要花比较多功夫来插入 `<br />` ，但是「每个换行都转换为 `<br />`」的方法在 Markdown 中并不适合， Markdown 中 email 式的 [块引言](https://theme-hope.vuejs.press/zh/cookbook/markdown/#blockquotes) 和多段落的 [列表](https://theme-hope.vuejs.press/zh/cookbook/markdown/#列表) 在使用换行来排版的时候，不但更好用，还更好阅读。

#### 1.2.2 标题

标题能显示出文章的结构。

Markdown 支持两种标题的语法，[Setextopen in new window](http://docutils.sourceforge.net/mirror/setext.html) 和 [atxopen in new window](http://www.aaronsw.com/2002/atx/) 形式。
...
**请注意:** 有一个已知的问题是 [Markdown.plopen in new window](http://markdown.pl/) 1.0.1 会忽略单引号包起来的链接 title。
...
*预设的链接标签*功能让你可以省略指定链接标签，这种情形下，链接标签和链接文字会视为相同，要用预设链接标签只要在链接文字后面加上一个空的方括号，如果你要让 "Google" 链接到 [google.comopen in new window](http://google.com/)，你可以简化成:
...
在浏览器里面，这段字串会变成一个可以点击的 [address@example.com](mailto:address@example.com) 链接。
...
你可以在 [Emoji 列表](https://theme-hope.vuejs.press/zh/cookbook/markdown/emoji/) 找到所有可用的 Emoji。
